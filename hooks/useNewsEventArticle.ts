...
const newsEventArticles: Record<string, NewsEventArticle> = {
  "1": {
    id: "1",
...
console.log("APIT" + eventSlug);
console.log(newsEventArticles[eventSlug]);

  return newsEventArticles[eventSlug] ?? null;
}
...
export function useNewsEventArticle(eventSlug?: string) {
  return useQuery({
    ...
    initialData: eventSlug ? newsEventArticles[eventSlug] : undefined,

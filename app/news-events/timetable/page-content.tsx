...
import { Clock, User, BookOpen, Download, Info } from "lucide-react";
        ...
        <header className="mb-20 flex flex-col  lg:flex-row justify-between items-start lg:items-end gap-10">
          ...
          <div className="flex flex-wrap gap-4 bg-gray-50 dark:bg-zinc-900 p-3 rounded-[2rem] border border-divider">
            ...
            <Select
              ...
              onSelectionChange={(keys) =>
                setSelectedSem(Array.from(keys)[0] as string)
              }
            >
            ...
            {FIXED_SLOTS.map((slot, idx) => (
              ...
              >
                ...
                {idx !== FIXED_SLOTS.length - 1 && (
                  <Divider className="absolute bottom-0 w-1/2 left-1/4 opacity-50" />
                )}
              </div>
            ))}
          </div>
          ...
          {data.days.map((day) => (
            <div key={day} className="flex flex-col gap-0">
              ...
              <div className="flex flex-col">
                {FIXED_SLOTS.map((slot, idx) => {
                  const session = filteredTimetable.find(
                    (s) => s.day === day && s.timeSlot === slot,
                  );
                  return (
                    <div
                      key={`${day}-${slot}`}
                      className="h-56 flex flex-col items-stretch relative py-4"
                    >
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        <Chip
          size="sm"
          className={`text-white font-black text-[8px] border-none ${chipColors[session.type]}`}
        >
          {session.type}
        </Chip>

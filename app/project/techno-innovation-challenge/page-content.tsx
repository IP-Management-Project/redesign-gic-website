...
import TICIncubationHub, {
  type TicHubSectionKey,
} from "@/components/incubation-roadmap";
import {
...
export default function TICShowcasePage({
  ...
  const seasons = allSeasons.filter((season) => {
    const hasText = [season.season, season.year, season.winner, season.desc]
      .filter(Boolean)
      .some((v) => String(v).trim().length > 0);
    const hasTeams = (season.teams ?? []).some(
      (team) => team && team.trim().length > 0,
    );
    return hasText || hasTeams;
  });
  ...
  }, [seasons.length, selectedSeason]);

  const safeSeasonIndex = seasons.length
    ? Math.min(selectedSeason, seasons.length - 1)
    : 0;
  const selectedSeasonData = seasons[safeSeasonIndex];
  ...
  const handleDeleteSeason = (index: number) => {
    const items = seasons ?? [];
    if (!items.length) return;

    const nextSeasons = items.filter((_, i) => i !== index);
    updateShowcase.mutate({
      section: `season-${index}`,
      data: { seasons: nextSeasons },
    });

    if (selectedSeason >= nextSeasons.length) {
      setSelectedSeason(Math.max(nextSeasons.length - 1, 0));
    }
  };
          ...
          <div className="pointer-events-none absolute inset-0">
            <div className="pointer-events-auto absolute right-6 top-6 z-20">
              <Button
                size="sm"
                variant="flat"
                onPress={getEditAction("launchpad")}
              >
                Edit launchpad
              </Button>
            </div>
            ...
            <motion.div {...fadeIn} className="grid grid-cols-2 gap-4">
              ...
              {launchpadSecondaryImage?.src ? (
                ...
                <img
                  ...
                  alt={
                    launchpadSecondaryImage.alt || "Incubation collaboration"
                  }
                />
              ) : (
      ...
      <section className="py-24 bg-zinc-950 text-white rounded-[4rem] mx-4 md:mx-6 overflow-hidden relative border border-[#c8c8c8]/10">
          ...
          <div className="pointer-events-none absolute inset-0">
            <div className="pointer-events-auto absolute right-6 top-6 z-20 flex flex-col gap-3">
              <Button
                size="sm"
                variant="flat"
                onPress={getEditAction("heritage")}
              >
                Edit heritage
              </Button>
              <Button
                ...
                size="sm"
                variant="bordered"
                onPress={getEditAction(`season-${seasons.length}`)}
              >
                Add season
              </Button>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <motion.div {...fadeIn} className="text-center mb-20">
            ...
            <p className="text-[#c8c8c8] max-w-2xl mx-auto text-lg">
              {heritage.subtitle}
            </p>
          </motion.div>
                  ...
                  {editable ? (
                    <div className="mb-6 flex justify-end gap-2">
                      ...
                      <Button
                        ...
                        variant="bordered"
                        color="danger"
                        onPress={() => handleDeleteSeason(safeSeasonIndex)}
                      >
                        Delete
                      </Button>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-4">
                      {selectedTeams.map((team) => (
                        ...
                        >
                          <Sparkles
                            className="inline-block mr-2 text-[#007d49] group-hover:animate-spin"
                            size={16}
                          />{" "}
                          {team}
                        </div>
          ...
          <div className="pointer-events-none absolute inset-0">
            <div className="pointer-events-auto absolute right-6 top-6 z-20 flex flex-col gap-3">
              ...
              <Button
                size="sm"
                variant="bordered"
                onPress={getEditAction(`gallery-item-${galleryItems.length}`)}
              >
                Add gallery item
              </Button>
            </div>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
            <motion.div {...fadeIn}>
              <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tighter flex items-center gap-4">
                ...
                {galleryCopy.titleMain}{" "}
                <span className="text-[#007d49]">
                  {galleryCopy.titleHighlight}
                </span>
              </h2>
                ...
                {editable ? (
                  <div className="absolute right-3 top-3 z-20 flex gap-2">
                    <Button
                      size="sm"
                      variant="flat"
                      onPress={getEditAction(`gallery-item-${index}`)}
                    >
                      Edit {index + 1}
                    </Button>
                    <Button
                      ...
                      variant="bordered"
                      color="danger"
                      onPress={() => handleDeleteGalleryItem(index)}
                    >
                      Delete
                    </Button>
                  </div>

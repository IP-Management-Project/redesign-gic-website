...
export function ErasmusMontusContent() {
  const locale = useLocale();
  const { summary, consortiumMeeting, training } = erasmusMontusData;
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
              {erasmusMontusData.title}
            </h1>
            <p className="text-gray-600 text-lg mb-2">
              {erasmusMontusData.tagline}
            </p>
            <p className="text-gray-500 text-sm">
              {erasmusMontusData.projectId}
            </p>
          </div>
        </div>
      </div>
        ...
        <section className="mb-16">
          ...
          <div className="space-y-4">
            {consortiumMeeting.meetings.map((meeting, index) => (
              ...
              >
                <Link
                  href={`/${locale}/projects/erasmus-montus/meeting/${meeting.slug}`}
        ...
        <section>
          ...
          <div className="space-y-4">
            {training.workshops.map((workshop, index) => (
              ...
              >
                <Link
                  href={`/${locale}/projects/erasmus-montus/training/${workshop.slug}`}
                  className="block border-l-4 border-gray-300 pl-6 py-3 hover:bg-gray-50 transition-colors rounded-r-lg group"
                >
                  <div className="flex items-start justify-between">
                    <div>
